...
function SettingsPage() {
  ...
  return (
    <div className="p-6">
        ...
        {tabs.map(t => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm ${
              tab === t.id ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
            }`}
          >
            {t.icon}
            {t.label}
      ...
      {tab === 'business' && (
        <div id="panel-business" role="tabpanel" aria-labelledby="tab-business" className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
      ...
      {tab === 'receptionist' && (
        <div id="panel-receptionist" role="tabpanel" aria-labelledby="tab-receptionist" className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Receptionist Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Greeting Template</label>
            <textarea
...
function ServicesManager() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6 space-y-4">
          <h4 className="font-medium text-gray-900 dark:text-gray-100">{editingId ? 'Edit Service' : 'Add Service'}</h4>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={cancelForm} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300">
              Cancel
            </button>
          </div>
          ...
          {previewText && (
            <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-blue-600 dark:text-blue-400 uppercase tracking-wide">How it looks to customers</span>
              </div>
              <p className="text-sm text-blue-900 dark:text-blue-200 whitespace-pre-line font-medium">
                {previewText}
              </p>
            </div>
          )}
      ...
      {isLoading ? (
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 animate-pulse">
              <div className="flex justify-between">
                <div><div className="h-4 w-32 bg-gray-200 dark:bg-gray-700 rounded" /><div className="h-3 w-20 bg-gray-100 dark:bg-gray-600 rounded mt-2" /></div>
        ...
        <div className="space-y-2">
          {services.map(svc => (
            <div key={svc.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(svc)} className="text-sm text-blue-500 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">Edit</button>
                <button onClick={() => handleDelete(svc.id)} className="text-sm text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">Delete</button>
              </div>
            </div>
          ))}
        </div>
...
function AvailabilityManager() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Business Hours</h3>
        {!showForm && (
          <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6 space-y-4">
          <h4 className="font-medium text-gray-900 dark:text-gray-100">Add Business Hours</h4>
          <div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300">Cancel</button>
          </div>
        </form>
      )}
          ...
          {rules.sort((a, b) => a.dayOfWeek - b.dayOfWeek).map(rule => (
            <div key={rule.id} className={`flex items-center justify-between p-4 rounded-lg border ${
              ...
              <div className="flex items-center gap-4">
                <span className="font-medium text-gray-900 dark:text-gray-100 w-28">{DAYS[rule.dayOfWeek] ?? `Day ${rule.dayOfWeek}`}</span>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-full ${
                    rule.available ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400'
                  }`}
                >
                  {rule.available ? 'Open' : 'Closed'}
                </button>
                <button onClick={() => handleDelete(rule.id)} className="text-sm text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">Delete</button>
              </div>
            </div>
          ))}
        </div>
