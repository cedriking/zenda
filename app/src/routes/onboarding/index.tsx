...
function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 flex">
      {/* Progress sidebar */}
      <aside
        className={`w-80 bg-white/80 backdrop-blur-sm border-r border-gray-100 p-8 flex flex-col transition-all duration-700 ease-out ${
      ...
      >
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Your AI receptionist</p>
          </div>
        </div>
              ...
              <div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-500 ${
                  isComplete
                    ...
                    : isCurrent
                      ? 'text-blue-700 bg-blue-50/80 font-medium shadow-sm'
                      : 'text-gray-300'
                }`}
                style={{
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-6 py-4 border-b border-gray-100 bg-white/60 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 text-sm">Setup Assistant</h3>
              <p className="text-xs text-gray-400">Answer a few questions to configure your AI receptionist</p>
            </div>
          </div>
        </div>
...
function ChatView({
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit} className="px-6 py-4 border-t border-gray-100 bg-white/80 backdrop-blur-sm">
        <div className="flex gap-3 items-center max-w-2xl mx-auto">
          <input
            ...
            className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-300 text-sm placeholder:text-gray-400 transition-all duration-200"
          />
          <button
            type="submit"
    ...
    <div className="flex-1 overflow-auto px-6 py-6 space-y-6">
      ...
      >
        {PLAN_TIERS.map((plan, idx) => {
          ...
          return (
            <div
              key={plan.id}
              className={`rounded-2xl border p-6 bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                plan.highlight
                  ? 'border-blue-400 shadow-lg shadow-blue-500/10 relative ring-1 ring-blue-400/20'
            ...
            >
              ...
              <div className="flex items-center gap-2.5 mb-2">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${plan.highlight ? 'bg-blue-50 text-blue-500' : 'bg-gray-50 text-gray-400'}`}>
                  <Icon size={18} />
                </div>
              ...
              <div className="mb-5">
                ...
                {plan.originalPrice && (
                  <span className="ml-2 text-sm text-gray-300 line-through">${plan.originalPrice}</span>
                )}
              </div>
      ...
      >
        <button
          onClick={onSkip}
          disabled={checkoutLoading !== null}
          className="text-sm text-gray-400 hover:text-gray-600 transition-colors duration-200 disabled:opacity-40 flex items-center gap-1.5"
        >
