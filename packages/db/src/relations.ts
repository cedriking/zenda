...
} from "./schema/business-profiles.js";
import {
...
import { subscriptions, usageRecords } from "./schema/subscriptions.js";
...
export const workspaceRelations = relations(workspaces, ({ many, one }) => ({
  ...
  whatsappConnection: one(whatsappConnections, {
    fields: [workspaces.id],
    references: [whatsappConnections.workspaceId],
  }),
  ...
  receptionistProfile: one(receptionistProfiles, {
    fields: [workspaces.id],
    references: [receptionistProfiles.workspaceId],
  }),
...
export const whatsappConnectionRelations = relations(
  ...
  ({ one }) => ({
    workspace: one(workspaces, {
      fields: [whatsappConnections.workspaceId],
      references: [workspaces.id],
    }),
  })
);
...
export const receptionistProfileRelations = relations(
  receptionistProfiles,
  ({ one }) => ({
    workspace: one(workspaces, {
      fields: [receptionistProfiles.workspaceId],
      references: [workspaces.id],
    }),
  })
);
...
export const staffMemberRelations = relations(
  staffMembers,
  ({ one, many }) => ({
    workspace: one(workspaces, {
      fields: [staffMembers.workspaceId],
      references: [workspaces.id],
    }),
    appointments: many(appointments),
    availabilityRules: many(availabilityRules),
...
export const availabilityRuleRelations = relations(
  ...
  ({ one }) => ({
    workspace: one(workspaces, {
      fields: [availabilityRules.workspaceId],
      references: [workspaces.id],
    }),
    staffMember: one(staffMembers, {
      fields: [availabilityRules.staffMemberId],
...
export const appointmentRelations = relations(appointments, ({ one }) => ({
  ...
  sourceConversation: one(conversations, {
    fields: [appointments.sourceConversationId],
...
export const agentMemoryRelations = relations(agentMemory, ({ one }) => ({
  workspace: one(workspaces, {
    fields: [agentMemory.workspaceId],
    references: [workspaces.id],
  }),
  customer: one(customers, {
    fields: [agentMemory.customerId],
    references: [customers.id],
  }),
}));
...
export const knowledgeBaseItemRelations = relations(
  ...
  ({ one }) => ({
    workspace: one(workspaces, {
      fields: [knowledgeBaseItems.workspaceId],
      references: [workspaces.id],
    }),
  })
);
...
export const providerUsageRelations = relations(providerUsage, ({ one }) => ({
  workspace: one(workspaces, {
    fields: [providerUsage.workspaceId],
    references: [workspaces.id],
  }),
}));
...
export const audioAssetRelations = relations(audioAssets, ({ one }) => ({
  message: one(messages, {
    fields: [audioAssets.messageId],
    references: [messages.id],
  }),
  workspace: one(workspaces, {
    fields: [audioAssets.workspaceId],
    references: [workspaces.id],
  }),
}));
