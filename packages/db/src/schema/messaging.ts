...
export const messagingConsent = pgTable(
  "messaging_consent",
  {
  ...
  (table) => [
    uniqueIndex("messaging_consent_workspace_customer_idx").on(
      table.workspaceId,
      table.customerId
    ),
...
export const outboundMessageLog = pgTable(
  "outbound_message_log",
  {
    ...
    outboundSinceLastInbound: integer("outbound_since_last_inbound")
      .notNull()
      .default(0),
  ...
  (table) => [
    uniqueIndex("outbound_message_log_workspace_customer_idx").on(
      table.workspaceId,
      table.customerId
    ),
  ]
...
export const sentReminderLog = pgTable(
  ...
  (table) => [
    uniqueIndex("sent_reminder_log_appointment_type_idx").on(
      table.appointmentId,
      table.reminderType
    ),
  ]
