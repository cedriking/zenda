...
async function reset() {
  ...
  const tables = await nukeSql`SELECT tablename FROM pg_tables WHERE schemaname = 'public'`;
  for (const t of tables) await nukeSql.unsafe(`DROP TABLE IF EXISTS "${t.tablename}" CASCADE`);
  const enums = await nukeSql`
    SELECT t.typname FROM pg_type t
    JOIN pg_namespace n ON t.typnamespace = n.oid
    WHERE n.nspname = 'public' AND t.typtype = 'e'
  `;
  for (const e of enums) await nukeSql.unsafe(`DROP TYPE IF EXISTS "${e.typname}" CASCADE`);
  console.log(`  Dropped ${tables.length} tables, ${enums.length} enums`);
  await nukeSql.end();

  // Step 2: Read migration SQL
  const migrationDir = path.join(__dirname, '..', 'packages', 'db', 'src', 'migrations');
  ...
  if (fs.existsSync(journalPath)) {
    ...
    for (const entry of journal.entries) {
      ...
      const sqlFiles = fs.readdirSync(migrationDir).filter(f => f.startsWith(entry.tag) && f.endsWith('.sql'));
      if (sqlFiles.length > 0) {
        const fileContent = fs.readFileSync(path.join(migrationDir, sqlFiles[0]), 'utf8');
        const hash = crypto.createHash('sha256').update(fileContent).digest('hex').substring(0, 16);
        await applySql.unsafe(
...
reset().catch(e => { console.error('FAILED:', e.message); process.exit(1); });
