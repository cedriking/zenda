...
function getGitInfo() {
  ...
  if (parts.length >= 4) {
    ...
    result.ahead = Number.parseInt(ab[0], 10) || 0;
    result.behind = Number.parseInt(ab[1], 10) || 0;
  }
      ...
      for (const [projectPath, projectConfig] of Object.entries(
        ...
        if (CWD === projectPath || CWD.startsWith(`${projectPath}/`)) {
          ...
          if (usage) {
            ...
            if (ids.length > 0) {
              let modelId = ids.at(-1);
              let latest = 0;
              for (const id of ids) {
                const ts = usage[id]?.lastUsedAt
  ...
  if (activityData?.swarm) {
    ...
    if (age < staleThresholdMs) {
      return {
        ...
        coordinationActive:
          activityData.swarm.coordination_active || activityData.swarm.active,
      };
    }
  }
...
function getAgentDBStats() {
  ...
  if (storeStat) {
    ...
    try {
      ...
      if (Array.isArray(store)) {
        ...
        vectorCount += store.entries.length;
      }
    } catch {
      /* fall back */
    }
  ...
  try {
    ...
    if (ranked?.entries && ranked.entries.length > vectorCount) {
...
function generateStatusline() {
  ...
  if (git.gitBranch) {
    ...
    if (changes > 0) {
      let ind = "";
      if (git.staged > 0) {
        ind += `${c.brightGreen}+${git.staged}${c.reset}`;
      }
      if (git.modified > 0) {
        ind += `${c.brightYellow}~${git.modified}${c.reset}`;
      }
      if (git.untracked > 0) {
        ind += `${c.dim}?${git.untracked}${c.reset}`;
      }
      header += ` ${ind}`;
    }
    if (git.ahead > 0) {
...
function getStdinData() {
  if (_stdinData !== undefined && _stdinData !== null) {
    return _stdinData;
  }
  try {
