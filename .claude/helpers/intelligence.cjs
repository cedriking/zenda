...
function computePageRank(nodes, edges, damping, maxIter) {
  ...
  for (const edge of edges) {
    if (outLinks[edge.sourceId]) {
      outLinks[edge.sourceId].push(edge.targetId);
    }
    if (inLinks[edge.targetId]) {
      inLinks[edge.targetId].push(edge.sourceId);
    }
  }
...
function consolidate() {
  ...
  for (const entry of store) {
    if (!entry.id) {
      entry.id = `entry-${Math.random().toString(36).slice(2, 8)}`;
    }
  }
  const edges = buildEdges(store);
...
function saveSnapshot(graph, ranked) {
  ...
  if (graph?.pageRanks) {
    for (const v of Object.values(graph.pageRanks)) {
      snap.pageRankSum += v;
    }
  }
  if (graph?.nodes) {
...
function stats(outputJson) {
  ...
  const confMax = confidences.length ? confidences.at(-1) : 0;
