...
describe("Audio handler utilities", () => {
  test("data URI parsing extracts correct base64 payload", async () => {
    ...
    const dataUri =
      "data:audio/ogg; codecs=opus;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEA";
  ...
  test("MIME extraction from data URI works for various formats", () => {
    ...
    expect(extractMimeType("data:audio/ogg; codecs=opus;base64,abc")).toBe(
      "audio/ogg"
    );
    ...
    expect(extractMimeType("https://example.com/audio.ogg")).toBe(
      "audio/ogg; codecs=opus"
    ); // URL fallback
    expect(extractMimeType("https://example.com/audio.ogg", "audio/webm")).toBe(
      "audio/webm"
    ); // URL with explicit
  });
  ...
  test("large base64 audio round-trips correctly", () => {
    ...
    const original = Buffer.alloc(102_400, 0xab);
