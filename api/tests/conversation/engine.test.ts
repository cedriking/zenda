...
describe("EMERGENCY_PATTERN - negative cases", () => {
  test("does not match normal message", () => {
    expect(EMERGENCY_PATTERN.test("I would like to book an appointment")).toBe(
      false
    );
  });
...
describe("detectAutoEscalation - low confidence English", () => {
  test('detects "I\'m not sure"', () => {
    expect(detectAutoEscalation("I'm not sure about that")).toBe(
      "low_confidence"
    );
  });

  test('detects "I don\'t know"', () => {
    expect(detectAutoEscalation("I don't know the answer")).toBe(
      "low_confidence"
    );
  });

  test('detects "I cannot help"', () => {
    expect(detectAutoEscalation("I cannot help with that")).toBe(
      "low_confidence"
    );
  });

  test('detects "beyond my scope"', () => {
    expect(detectAutoEscalation("This is beyond my scope")).toBe(
      "low_confidence"
    );
  });
...
describe("detectAutoEscalation - tool failure", () => {
  test("detects tool_call_failed in tool calls", () => {
    const toolCalls = [
      { name: "book_appointment", result: "tool_call_failed" },
    ];
    expect(detectAutoEscalation("Here is the result", toolCalls)).toBe(
      "technology_question"
    );
  });
  ...
  test("detects function_call_error in tool calls", () => {
    ...
    expect(detectAutoEscalation("Normal response", toolCalls)).toBe(
      "technology_question"
    );
  });
...
describe("detectAutoEscalation - no escalation needed", () => {
  test("returns null for confident response", () => {
    expect(
      detectAutoEscalation("Your appointment is confirmed for tomorrow at 3pm")
    ).toBeNull();
  });
