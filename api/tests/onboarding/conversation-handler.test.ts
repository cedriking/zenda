...
function mapCategory(response: string): string {
  ...
  if (/clinic|clínica|medic|doctor|health|salud/i.test(lower)) return 'health'
  return 'other'
}
...
function parseReceptionistConfig(input: string): { name: string; tone: string } {
  let name = 'Noa'
...
describe('Availability parsing', () => {
  test('Mon-Fri 9am-6pm', () => {
    ...
    expect(result[0]).toEqual({ dayOfWeek: 1, startTime: '09:00', endTime: '18:00' })
    expect(result[4]).toEqual({ dayOfWeek: 5, startTime: '09:00', endTime: '18:00' })
  })

  test('Mon-Fri 9am-6pm, Saturday 10am-2pm', () => {
    const result = parseAvailability('Monday-Friday 9am-6pm, Saturday 10am-2pm')
    expect(result).toHaveLength(6)
    expect(result[5]).toEqual({ dayOfWeek: 6, startTime: '10:00', endTime: '14:00' })
  })
