...
export function enqueueSafe(action: string, payload: Record<string, unknown>): string {
  ...
  const item: OfflineItem = { id, type: 'safe', action, payload, createdAt: Date.now() }
...
export function enqueueUnsafe(action: string, payload: Record<string, unknown>): string {
  ...
  const item: OfflineItem = { id, type: 'unsafe', action, payload, createdAt: Date.now() }
  memoryCache.push(item)
...
export function getQueuedItems(type?: QueueType): OfflineItem[] {
  if (type) return memoryCache.filter(i => i.type === type)
  return [...memoryCache]
}
...
export async function flushPersistentQueue(
  workspaceId: string,
  sendFn: (msg: { id: string; conversationId: string; body: string; contentType: string }) => Promise<boolean>,
): Promise<{ sent: number; failed: number }> {
  let sent = 0
  let failed = 0
  ...
  try {
    ...
    logger.error('Persistent queue flush failed', { error: (err as Error).message })
  }
