    ...
    try {
      ...
      if (success) {
        ...
        if (msg.attempts >= msg.maxAttempts) {
          retryQueue.splice(i, 1);
          failed++;
          logger.error("Retry exhausted, persisting message to DB", {
            id: msg.id,
          ...
          msg.nextRetry =
            now + Math.min(BASE_DELAY * 2 ** msg.attempts, MAX_DELAY);
          logger.warn("Retry failed, rescheduling", {
            id: msg.id,
            attempts: msg.attempts,
          });
        }
      ...
      msg.attempts++;
      msg.nextRetry = now + Math.min(BASE_DELAY * 2 ** msg.attempts, MAX_DELAY);
    }
  }
