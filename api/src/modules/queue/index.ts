...
export const queueModule = new Elysia({ prefix: '/queue' })
  ...
  .post('/kill-switch', async ({ body, set }) => {
    try {
      ...
      logger.error('Failed to toggle kill switch', { error: (err as Error).message })
  ...
  .get('/dead-letters', async ({ query, set }) => {
    try {
      ...
      logger.error('Failed to get dead letters', { error: (err as Error).message })
  ...
  .post('/dead-letters/:id/retry', async ({ params, set }) => {
    try {
      ...
      if (!result) {
        return notFound(set, 'Dead letter message not found or not in dead_letter status')
      }
      return { status: 'retried', message: result }
    } catch (err) {
      logger.error('Failed to retry dead letter', { error: (err as Error).message })
