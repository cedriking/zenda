...
import { appointments, customers, messagingConsent } from '@zenda/db/schema'
import { eq, and } from 'drizzle-orm'
import { getPendingMessages, markFailed, type QueuedMessage } from './persistent-queue.js'
 ...
 */
export async function revalidateQueue(workspaceId: string): Promise<RevalidationResult> {
  ...
  for (const msg of pending) {
    ...
    await markFailed(msg.id, validation.reason)
    dropped++
    reasons[validation.reason] = (reasons[validation.reason] ?? 0) + 1

    const isImportant = ['appointment_cancelled', 'appointment_time_passed'].includes(validation.reason)
    if (isImportant) {
...
async function validateMessage(
  ...
  if (msg.metadata?.dedupeKey) {
    ...
    if (msg.appointmentId) {
      dedupeConditions.push(eq_(outboundQueue.appointmentId, msg.appointmentId))
    } else {
