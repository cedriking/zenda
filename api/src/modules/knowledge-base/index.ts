...
export const knowledgeBaseModule = new Elysia({ prefix: '/knowledge-base' })
  ...
  .get('/', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get knowledge base', { error: (err as Error).message })
  ...
  .get('/search', async ({ workspaceId, query, set }) => {
    try {
      ...
      logger.error('Failed to search knowledge base', { error: (err as Error).message })
  ...
  .post('/', async ({ workspaceId, body, set }) => {
    try {
      ...
      logger.error('Failed to create knowledge item', { error: (err as Error).message })
  ...
  .delete('/:id', async ({ workspaceId, params, set }) => {
    try {
      ...
      logger.error('Failed to delete knowledge item', { error: (err as Error).message })
