...
export class ComposioClient {
  ...
  async connectAccount(
    workspaceId: string
  ): Promise<{ connectionUrl: string; sessionId: string }> {
    try {
  ...
  async handleCallback(
    ...
    try {
      ...
      throw new Error(
        `Failed to complete connection: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async updateEvent(
    ...
    try {
      ...
      logger.error("Composio update event failed", {
        error,
        workspaceId,
        eventId,
      });
  ...
  async deleteEvent(
    ...
    try {
      ...
      logger.error("Composio delete event failed", {
        error,
        workspaceId,
        eventId,
      });
  ...
  async getConnectionStatus(
    ...
    try {
      ...
      logger.error("Composio get connection status failed", {
        error,
        workspaceId,
      });
      return null;
  ...
  async disconnectAccount(
    ...
    try {
      ...
      logger.error("Composio disconnect account failed", {
        error,
        workspaceId,
      });
      throw new Error(
...
export function getComposioClient(): ComposioClient {
  if (!composioClientInstance) {
    ...
    if (!config.apiKey) {
      logger.warn(
        "COMPOSIO_API_KEY not set, Composio integration will be disabled"
      );
    }
