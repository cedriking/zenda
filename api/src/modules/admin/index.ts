...
export const adminModule = new Elysia({ prefix: '/admin' })
  ...
  .get('/workspaces', async ({ set }) => {
    try {
      ...
      logger.error('Failed to list workspaces', { error: (err as Error).message })
  ...
  .get('/workspaces/:id', async ({ params, set }) => {
    try {
      ...
      logger.error('Failed to get workspace detail', { error: (err as Error).message })
  ...
  .get('/subscriptions', async ({ set }) => {
    try {
      ...
      logger.error('Failed to list subscriptions', { error: (err as Error).message })
  ...
  .post('/workspaces/:id/override-plan', async ({ params, body, set }) => {
    try {
      ...
      logger.error('Failed to override plan', { error: (err as Error).message })
  ...
  .get('/stats', async ({ set }) => {
    try {
      ...
      logger.error('Failed to get admin stats', { error: (err as Error).message })
