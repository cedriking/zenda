...
import { updateBusinessProfileSchema, updateReceptionistProfileSchema } from '@zenda/shared'
...
export const businessModule = new Elysia({ prefix: '/business' })
  ...
  .get('/profile', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get business profile', { error: (err as Error).message })
      return serverError(set, 'Failed to get business profile')
    }
  })

  // Update business profile
  .patch('/profile', async ({ workspaceId, body, set }) => {
    try {
      ...
      logger.error('Failed to update business profile', { error: (err as Error).message })
  ...
  .get('/receptionist', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get receptionist profile', { error: (err as Error).message })
  ...
  .patch('/receptionist', async ({ workspaceId, body, set }) => {
    try {
      ...
      logger.error('Failed to update receptionist profile', { error: (err as Error).message })
