...
async function getWorkspaceSetting(workspaceId: string, key: string): Promise<string | null> {
  const [row] = await db
...
export const settingsModule = new Elysia({ prefix: '/settings' })
  ...
  .get('/receptionist', async ({ workspaceId, set }) => {
    try {
      const [profile] = await db
        ...
        .limit(1)
      if (!profile) return notFound(set, 'Receptionist profile not found')
      return profile
    } catch (err) {
      logger.error('Failed to get receptionist settings', { error: (err as Error).message })
  ...
  .patch('/receptionist', async ({ workspaceId, body, set }) => {
    try {
      ...
      logger.error('Failed to update receptionist settings', { error: (err as Error).message })
  ...
  .get('/appointments', async ({ workspaceId, set }) => {
    try {
      const [profile] = await db
        ...
        .leftJoin(receptionistProfiles, eq(receptionistProfiles.workspaceId, businessProfiles.workspaceId))
        .where(eq(businessProfiles.workspaceId, workspaceId!))
        .limit(1)
      if (!profile) return notFound(set, 'Business profile not found')
      return profile
    } catch (err) {
      logger.error('Failed to get appointment settings', { error: (err as Error).message })
      return serverError(set, 'Failed to get appointment settings')
    }
  })

  .patch('/appointments', async ({ workspaceId, body, set }) => {
    try {
      const data = body as Record<string, unknown>
      ...
      if ('cancellationWindowHours' in data) businessFields.cancellationWindowHours = data.cancellationWindowHours
      if ('reschedulingWindowHours' in data) businessFields.reschedulingWindowHours = data.reschedulingWindowHours
      if ('depositRequired' in data) businessFields.depositRequired = data.depositRequired
      if ('depositAmountCents' in data) businessFields.depositAmountCents = data.depositAmountCents
      if ('approvedCancellationText' in data) businessFields.approvedCancellationText = data.approvedCancellationText
      if ('approvedRefundText' in data) businessFields.approvedRefundText = data.approvedRefundText
      if ('cancellationPolicyStrictness' in data) receptionistFields.cancellationPolicyStrictness = data.cancellationPolicyStrictness
      ...
      if (Object.keys(businessFields).length > 0) {
        const [updated] = await db
          ...
          .where(eq(businessProfiles.workspaceId, workspaceId!))
          .returning()
        if (!updated) return notFound(set, 'Business profile not found')
      ...
      logger.error('Failed to update appointment settings', { error: (err as Error).message })
  ...
  .get('/safety', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get safety settings', { error: (err as Error).message })
      return serverError(set, 'Failed to get safety settings')
    }
  })

  .patch('/safety', async ({ workspaceId, body, set }) => {
    try {
      const data = body as Record<string, unknown>
      ...
      logger.error('Failed to update safety settings', { error: (err as Error).message })
  ...
  .get('/safety/escalations', async ({ workspaceId, query, set }) => {
    try {
      ...
      logger.error('Failed to get escalations', { error: (err as Error).message })
  ...
  .get('/messaging', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get messaging settings', { error: (err as Error).message })
      return serverError(set, 'Failed to get messaging settings')
    }
  })

  .patch('/messaging', async ({ workspaceId, body, set }) => {
    try {
      const data = body as Record<string, unknown>
      if ('maxOutboundWithoutReply' in data) {
        await setWorkspaceSetting(workspaceId!, 'maxOutboundWithoutReply', String(data.maxOutboundWithoutReply))
      }
      if ('maxRemindersPerAppointment' in data) {
        await setWorkspaceSetting(workspaceId!, 'maxRemindersPerAppointment', String(data.maxRemindersPerAppointment))
      }
      return {
        maxOutboundWithoutReply: Number(data.maxOutboundWithoutReply ?? (await getWorkspaceSetting(workspaceId!, 'maxOutboundWithoutReply')) ?? 5),
        maxRemindersPerAppointment: Number(data.maxRemindersPerAppointment ?? (await getWorkspaceSetting(workspaceId!, 'maxRemindersPerAppointment')) ?? 2),
      }
    } catch (err) {
      logger.error('Failed to update messaging settings', { error: (err as Error).message })
  ...
  .get('/messaging/consent', async ({ workspaceId, query, set }) => {
    try {
      ...
      logger.error('Failed to get messaging consent', { error: (err as Error).message })
