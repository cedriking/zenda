...
export const analyticsModule = new Elysia({ prefix: '/analytics' })
  ...
  .get('/', async ({ workspaceId, query, set }) => {
    try {
      ...
      const days = Math.max(1, Math.min(365, parseInt(period ?? '30') || 30))
      const end = new Date()
  ...
  .get('/messages-today', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get messages today', { error: (err as Error).message })
  ...
  .get('/dashboard-stats', async ({ workspaceId, set }) => {
    try {
      ...
      logger.error('Failed to get dashboard stats', { error: (err as Error).message })
