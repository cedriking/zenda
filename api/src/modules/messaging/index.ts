...
import { getConsent, optOut, recordConsent } from "./consent-service.js";

const MESSAGING_CONSENT_STATUSES: MessagingConsentStatus[] = [
  "unknown",
  "allowed",
  "limited",
  "opted_out",
...
export const messagingModule = new Elysia({ prefix: "/messaging" })
  ...
  .get("/consent/:customerId", async ({ workspaceId, params, set }) => {
    try {
      ...
      logger.error("Get consent error", {
        workspaceId,
  ...
  .post(
    ...
    },
    {
      body: t.Object({
        ...
        allowedPurposes: t.Optional(
          t.Array(t.Union(MESSAGE_PURPOSES.map((p) => t.Literal(p))))
        ),
