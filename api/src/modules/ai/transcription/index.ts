...
export async function transcribeAudio(
  ...
  for (const provider of providers) {
    try {
      ...
      console.warn(
        `[transcription] ${provider.name} failed:`,
...
async function transcribeWithLocal(
  ...
  if (!baseUrl) {
    throw new Error("WHISPER_LOCAL_URL not configured");
  }
  ...
  const response = await fetch(`${baseUrl}/transcribe`, {
    method: "POST",
    body: formData,
    signal: AbortSignal.timeout(60_000), // 60s timeout for large audio
  });
...
async function transcribeWithOpenAI(
  ...
  formData.append(
    "file",
    new Blob([new Uint8Array(audioBuffer)], { type: mime }),
    filename
  );
  ...
  if (data.segments && data.segments.length > 0) {
    const avgLogProb =
      data.segments.reduce((sum, s) => sum + s.avg_logprob, 0) /
