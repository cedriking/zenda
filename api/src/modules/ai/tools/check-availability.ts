...
export async function checkAvailability(
  ...
  for (const appt of existingAppts) {
    ...
    for (const slot of slots) {
      ...
      const slotStartMs = new Date(input.date + `T${slot.startTime}:00`).getTime()
  ...
  return { slots: slots.filter(s => s.available), serviceName: service.name, date: input.date }
}
...
export const checkAvailabilityToolDef = {
  ...
  function: {
    ...
    description: 'Check available time slots for a service on a specific date. Returns available slots.',
    parameters: {
      type: 'object',
      properties: {
        serviceId: { type: 'string', description: 'The service ID' },
        date: { type: 'string', description: 'Date in YYYY-MM-DD format' },
        staffMemberId: { type: 'string', description: 'Optional specific staff member ID' },
      },
