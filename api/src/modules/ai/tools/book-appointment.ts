...
type BookResult = NeedsInfoResult | AwaitingConfirmationResult | BookingResult
...
export async function bookAppointment(
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      question: questions[field] ?? `Could you let me know your preferred ${field === 'date' ? 'date' : field === 'startTime' ? 'time' : 'service'}?`,
    }
  }
  ...
  if (!service) {
    return {
      ...
      question: 'I couldn\'t find that service. Could you tell me which service you\'re looking for?',
    }
  }
  ...
  if (input.staffMemberId) {
    const [staff] = await db
      ...
      .from(staffMembers)
      .where(and(eq(staffMembers.id, input.staffMemberId), eq(staffMembers.workspaceId, workspaceId)))
      .limit(1)
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        serviceId: { type: 'string', description: 'Service ID (optional on first call)' },
        date: { type: 'string', description: 'Date in YYYY-MM-DD format (optional on first call)' },
        startTime: { type: 'string', description: 'Start time in HH:mm format (optional on first call)' },
        staffMemberId: { type: 'string', description: 'Optional staff member preference' },
        notes: { type: 'string', description: 'Optional notes' },
        confirmed: { type: 'boolean', description: 'Set true after the customer confirms the details' },
      },
      required: ['customerId'],
    },
  },
