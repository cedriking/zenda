...
function normalizeReason(trigger: string, fallbackReason: EscalationReason): EscalationReason {
  return ESCALATION_TRIGGER_MAP[trigger] ?? fallbackReason
}

function getEscalationResponse(reason: EscalationReason, language: 'en' | 'es' = 'es'): string {
  if (reason === 'emergency') {
    return language === 'es'
  ...
  const responses: Record<'en' | 'es', string> = language === 'es'
    ? { en: '', es: 'Voy a conectarle con alguien del equipo que puede ayudarle con eso. Un momento, por favor.' }
    : { en: "Let me connect you with someone from the team who can help with that. One moment, please.", es: '' }
