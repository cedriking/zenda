...
const INTENT_PATTERNS: IntentPattern[] = [
  // Opt-out must be checked early — "stop" is short and could overlap
  {
    intent: "opt_out",
    pattern:
      /\b(?:stop|unsubscribe|no\s+more|don't\s+(?:text|message|send)|detener|no\s+m[aá]s|basta|ya\s+no)\b/i,
    confidence: 0.95,
  },
  // Human escalation
  {
    intent: "ask_human",
    pattern:
      /\b(?:human|person|agent|speak\s+to\s+someone|talk\s+to\s+someone|someone\s+else|real\s+person|humano|persona|agente|hablar\s+con\s+alguien|hablar\s+alguien|alguien\s+m[aá]s|atenci[oó]n\s+humana)\b/i,
    confidence: 0.9,
  },
  // Cancel
  {
  ...
  // Reschedule
  {
    intent: "reschedule",
    pattern:
      /\b(?:reschedule|change\s+(?:the\s+)?time|change\s+(?:the\s+)?date|move\s+(?:my\s+)?appointment|reprogramar|cambiar\s+(?:la\s+)?hora|cambiar\s+(?:la\s+)?fecha|reagendar|cambiar\s+cita)\b/i,
    confidence: 0.9,
  },
  // Confirm
  {
    intent: "confirm",
    pattern:
      /\b(?:confirm|yes\s+confirm|confirmo|confirmar|s[ií].*confirmar|confirmar.*cita|claro|por\s+supuesto|dale|ok|okay|perfecto)\b/i,
    confidence: 0.75,
  },
  // Book / schedule
  {
    intent: "book",
    pattern:
      /\b(?:book|appointment|schedule|reserv(?:e|ar)|cita|agendar|quiero.*cita|necesito.*cita|pedir.*cita|solicitar.*cita|hacer\s+una\s+cita|make\s+(?:an?\s+)?appointment|set\s+up|quiero\s+agendar|me\s+gustar[ií]a.*cita)\b/i,
    confidence: 0.88,
  },
  // Check availability
  {
    ...
    pattern:
      /\b(?:available|free\s+slot|openings?|when\s+(?:can|is)|disponible|libre|horario(?:s)?\s+disponible|qu[eé]\s+(?:horas|d[ií]as)|hay\s+lugar|hay\s+espacio)\b/i,
    confidence: 0.85,
  },
  // Ask price
  {
    intent: "ask_price",
    pattern:
      /\b(?:price|cost|how\s+much|pricing|tariff|rate|cu[aá]nto(?:\s+(?:cuesta|sale|es|val[eé]))?|precio|costo|tarifa)\b/i,
    confidence: 0.9,
  },
  // Ask location
  {
    intent: "ask_location",
    pattern:
      /\b(?:where\s+(?:are|is|do)|location|address|directions|direcci[oó]n|d[oó]nde\s+(?:est[aá]n|est[aá]|quedan|queda|son)|c[oó]mo\s+llego)\b/i,
    confidence: 0.9,
  },
  // Ask hours
  {
    intent: "ask_hours",
    pattern:
      /\b(?:hours|open(?:ing)?|close|schedule|business\s+hours|horario(?:s)?|abierto|cerrado|a\s+qu[eé]\s+hora|cuando\s+abren)\b/i,
    confidence: 0.85,
  },
  // Reminder request
  {
    intent: "ask_reminder",
    pattern:
      /\b(?:remind(?:er)?|alert|notify|record(?:ar|atorio)|avisar|notificar|aviso|recordarme)\b/i,
    confidence: 0.85,
  },
];
...
function extractEntities(message: string): Record<string, string> {
  ...
  if (slashDate) {
    entities.date = `${slashDate[3].length === 2 ? `20${slashDate[3]}` : slashDate[3]}-${slashDate[2].padStart(2, "0")}-${slashDate[1].padStart(2, "0")}`;
  }
  ...
  const informalTime = message.match(
    /\b(?:a\s+las?\s+|at\s+)(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i
  );
...
const SHORT_CONFIRM_WORDS =
  /^(yes|no|ok|okay|yep|nope|si|sí|claro|dale|vale|perfecto|sure|right|correct|confirmo|exacto)$/i;
...
export function classifyIntent(message: string): IntentResult {
  ...
  if (SHORT_CONFIRM_WORDS.test(normalized)) {
    const affirmative =
      /^(yes|yep|si|sí|claro|dale|vale|perfecto|sure|right|correct|confirmo|exacto|ok|okay)$/i;
    const intent: ClassifiedIntent = affirmative.test(normalized)
      ? "confirm"
      : "cancel";
    return {
      intent,
