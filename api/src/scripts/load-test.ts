...
async function signupWorkspace(idx: number): Promise<{ token: string; workspaceId: string }> {
...
async function simulateWorkspace(token: string, _idx: number): Promise<{ success: number; errors: number }> {
...
async function main() {
  console.log(`Starting load test: ${CONCURRENT_WORKSPACES} workspaces x ${REQUESTS_PER_WORKSPACE} requests`)
  ...
  console.log(`Avg response time: ${((duration * 1000) / totalRequests).toFixed(0)}ms`)
}
